...
contract WorldBurnerGame is Blacklistable {
  bytes32 public name;
  uint public tokenId = 1002567;
  ...
  function setDiv(uint8 d) isOwner {
    require(div == 0 || div >= 2);
    div = d;
  }
  ...
  function register(address referralAddress) external payable okay {
      ...
      owner.transfer(buyIn * 1000000 / 20); // Send 5% of buy-in to the parent contract.
      if (streamTo != 0x0) {
        if (div != 0)
      ...
      if (totalPlayers < 10) {
        ...
        require(th.tokenBalance(tokenId) >= regBonus);
        msg.sender.transferToken(regBonus, tokenId);
      }
      totalPlayers++;
  ...
  function regBonus() public view returns (uint) {
    return (1000 * buyIn) * (10 - totalPlayers) * 100000;
  }
